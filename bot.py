...
def run_strategy_backtest(symbol, timeframe):
    ...
    summary = (
        ...
        "==================================================\n"
        f"Symbol:               {symbol}\n"
        f"Total Setups Formed:  {total_setups}\n"
        f"Executed Trades:      {total_trades}\n"
        f"Wins (1:2 RR):        {wins}\n"
        f"Breakevens (1:1 RR):  {breakevens}\n"
        f"Losses:               {losses}\n"
        "--------------------------------------------------\n"
        f"Win Rate (Wins + BE): {winrate:.2f}%\n"
        f"Total R Profit:       {total_r:.2f}R\n"
        f"Total Gold Profit:    ${total_points:.2f}\n"
        "=================================================="
    )
    
    print(summary)
    send_discord_alert("📊 Backtest Performance Summary", f"```\n{summary}\n```", 0x3498DB)
...
def check_live_signals():
    ...
    if is_bearish1 and is_bearish2 and is_bearish3:
        ...
        msg = f"**Bearish Doji Retest Setup Detected!**\n\n• **Asset:** XAUUSD (Gold)\n• **Timeframe:** {TIMEFRAME}\n• **Retest Sell Entry:** ${entry_price:.2f}\n• **Stop Loss:** ${sl_price:.2f}\n• **1:1 BE Trigger:** ${be_price:.2f}\n• **1:2 Target (TP):** ${tp_price:.2f}"
        send_discord_alert("🚨 Bearish Gold Setup Formed", msg, 0xFF0000)
        last_alert_time = c3_time
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Bearish signal sent!")
    ...
    if is_bullish1 and is_bullish2 and is_bullish3:
        ...
        msg = f"**Bullish Doji Retest Setup Detected!**\n\n• **Asset:** XAUUSD (Gold)\n• **Timeframe:** {TIMEFRAME}\n• **Retest Buy Entry:** ${entry_price:.2f}\n• **Stop Loss:** ${sl_price:.2f}\n• **1:1 BE Trigger:** ${be_price:.2f}\n• **1:2 Target (TP):** ${tp_price:.2f}"
        send_discord_alert("🚀 Bullish Gold Setup Formed", msg, 0x00FF00)
        last_alert_time = c3_time
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Bullish signal sent!")
...
if __name__ == "__main__":
    ...
    print(f"\n[LIVE MONITORING ACTIVATED] Scanning {SYMBOL} ({TIMEFRAME}) every {CHECK_INTERVAL_SEC}s...")
    while True:
        try:
            check_live_signals()
        except Exception as e:
